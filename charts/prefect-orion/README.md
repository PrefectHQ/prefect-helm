...
| ingress.tlsSecretName | string | `""` | Optionally specify TLS secret name. If empty, default value is `{{- printf "%s-tls" .Values.ingress.host.hostname }}` |
...
| postgresql.auth.password | string | `""` | password for the custom user to create. Ignored if `auth.existingSecret` is provided |
