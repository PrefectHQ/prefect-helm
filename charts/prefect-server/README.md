...
| loopServices.affinity | object | `{}` | affinity for server pods assignment |
| loopServices.autoscaling.enabled | bool | `false` | enable autoscaling for server |
| loopServices.autoscaling.maxReplicas | int | `100` | maximum number of server replicas |
| loopServices.autoscaling.minReplicas | int | `1` | minimum number of server replicas |
| loopServices.autoscaling.targetCPU | int | `80` | target CPU utilization percentage |
| loopServices.autoscaling.targetMemory | int | `80` | target Memory utilization percentage |
| loopServices.containerSecurityContext.allowPrivilegeEscalation | bool | `false` | set server containers' security context allowPrivilegeEscalation |
| loopServices.containerSecurityContext.capabilities | object | `{}` | set server container's security context capabilities |
| loopServices.containerSecurityContext.readOnlyRootFilesystem | bool | `true` | set server containers' security context readOnlyRootFilesystem |
| loopServices.containerSecurityContext.runAsNonRoot | bool | `true` | set server containers' security context runAsNonRoot |
| loopServices.containerSecurityContext.runAsUser | int | `1001` | set server containers' security context runAsUser |
| loopServices.debug | bool | `false` | enable server debug mode |
| loopServices.env | list | `[]` | array with environment variables to add to server nodes |
| loopServices.extraArgs | list | `[]` | array with extra Arguments for the server container to start with |
| loopServices.extraContainers | list | `[]` | additional sidecar containers |
| loopServices.extraEnvVarsCM | string | `""` | name of existing ConfigMap containing extra env vars to add to server nodes |
| loopServices.extraEnvVarsSecret | string | `""` | name of existing Secret containing extra env vars to add to server nodes |
| loopServices.extraVolumeMounts | list | `[]` | array with extra volumeMounts for the server pod |
| loopServices.extraVolumes | list | `[]` | array with extra volumes for the server pod |
| loopServices.image.prefectTag | string | `"3-latest"` | prefect image tag (immutable tags are recommended) |
| loopServices.image.pullPolicy | string | `"IfNotPresent"` | server image pull policy |
| loopServices.image.pullSecrets | list | `[]` | server image pull secrets |
| loopServices.image.repository | string | `"prefecthq/prefect"` | server image repository |
...
| loopServices.loopServices.enabled | bool | `false` | enable running services in a separate deployment |
| loopServices.loopServices.replicaCount | int | `1` | number of services replicas to deploy |
| loopServices.loopServices.resources | object | `{"limits":{"cpu":"1","memory":"1Gi"},"requests":{"cpu":"500m","memory":"512Mi"}}` | resources for the services container |
| loopServices.nodeSelector | object | `{}` | node labels for server pods assignment |
| loopServices.podAnnotations | object | `{}` | extra annotations for server pod |
| loopServices.podLabels | object | `{}` | extra labels for server pod |
| loopServices.podSecurityContext.fsGroup | int | `1001` | set server pod's security context fsGroup |
| loopServices.podSecurityContext.runAsNonRoot | bool | `true` | set server pod's security context runAsNonRoot |
| loopServices.podSecurityContext.runAsUser | int | `1001` | set server pod's security context runAsUser |
| loopServices.prefectApiHost | string | `"0.0.0.0"` | sets PREFECT_SERVER_API_HOST |
| loopServices.prefectApiUrl | string | `"http://localhost:4200/api"` | sets PREFECT_API_URL |
| loopServices.priorityClassName | string | `""` | priority class name to use for the server pods; if the priority class is empty or doesn't exist, the server pods are scheduled without a priority class |
...
| loopServices.replicaCount | int | `1` | number of server replicas to deploy |
| loopServices.resources.limits | object | `{"cpu":"1","memory":"1Gi"}` | the requested limits for the server container |
| loopServices.resources.requests | object | `{"cpu":"500m","memory":"512Mi"}` | the requested resources for the server container |
| loopServices.revisionHistoryLimit | int | `10` | the number of old ReplicaSets to retain to allow rollback |
| loopServices.runSeparately | bool | `false` | Deploy loop services in a separate deployment from the server https://github.com/PrefectHQ/prefect/tree/main/src/prefect/server/services |
| loopServices.tolerations | list | `[]` | tolerations for server pods assignment |
| loopServices.uiConfig.enabled | bool | `true` | set PREFECT_UI_ENABLED; enable the UI on the server |
| loopServices.uiConfig.prefectUiApiUrl | string | `""` | sets PREFECT_UI_API_URL |
| loopServices.uiConfig.prefectUiStaticDirectory | string | `"/ui_build"` | sets PREFECT_UI_STATIC_DIRECTORY |
| loopServices.uiConfig.prefectUiUrl | string | `""` | sets PREFECT_UI_URL |
...
| server.loopServices.enabled | bool | `false` | enable running services in a separate deployment |
| server.loopServices.replicaCount | int | `1` | number of services replicas to deploy |
| server.loopServices.resources | object | `{"limits":{"cpu":"1","memory":"1Gi"},"requests":{"cpu":"500m","memory":"512Mi"}}` | resources for the services container |
