...
| server.autoscaling.enabled | bool | `true` | enable autoscaling for server |
...
| server.autoscaling.targetCPU | int | `90` | target CPU utilization percentage |
| server.autoscaling.targetMemory | int | `90` | target Memory utilization percentage |
...
| server.resources.limits | object | `{"cpu":"1","memory":"512Mi"}` | the requested limits for the server container |
| server.resources.requests | object | `{"cpu":"500m","memory":"256Mi"}` | the requested resources for the server container |
