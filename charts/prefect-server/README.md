...
Self-hosted Prefect servers can be equipped with Basic Authentication string for an administrator/password combination.

The format of the auth string is `admin:<password>` (no brackets).
...
```sh
kubectl create secret generic prefect-server-basic-auth --from-literal=auth-string='admin:my-password'
```
...
server:
  basicAuth:
    enabled: true
    existingSecret: prefect-server-basic-auth
```
